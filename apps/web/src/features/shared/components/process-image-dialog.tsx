...
import { NumberInput } from "@akabase/ui/components/number-input";
import { Select } from "@akabase/ui/components/select";
import { SkeletonText } from "@akabase/ui/components/skeleton";
import { Slider } from "@akabase/ui/components/slider";
...
function ImageCompareViewer({
  ...
  const isComparing = Boolean(processedUrl);
...
function ResizeSection({
  ...
  const handleWidthChange = ({ valueAsNumber: newWidth }: { valueAsNumber: number }) => {
    if (Number.isNaN(newWidth) || newWidth < 1) {
      return;
    }
    onResizeChange(
      keepAspectRatio
...
function ProcessImageDialogContent({
  ...
  const exceedsMaxSize = processedImage != null && processedImage.blob.size > MAX_FILE_SIZE;
    ...
    <Dialog.Content maxH="95vh" display="grid" gridTemplateRows="auto minmax(0, 1fr) auto">
      ...
      <Dialog.Footer>
        ...
        <Button
          onClick={handleSave}
          loading={isPending}
          disabled={isPending || !originalImage || exceedsMaxSize}
          size="md"
          variant="solid"
        >
          適用
