...
import {
  createProjectCategoryInputSchema,
  useCreateProjectCategoryMutationOption,
...
export function CreateProjectCategoryDialog({
  ...
  defaultOpen,
  onClose,
}: CreateProjectCategoryDialogProps) {
  const [open, setOpen] = useState(defaultOpen ?? false);
  const { mutateAsync } = useMutation(useCreateProjectCategoryMutationOption());
  ...
  const form = useForm({
    ...
    validators: {
      onDynamic: createProjectCategoryInputSchema.omit({ eventId: true }),
      ...
      <Portal>
        ...
        <Dialog.Positioner>
          <Dialog.Content asChild>
            ...
            >
              <Dialog.Header>
                <Dialog.Title>企画区分を追加</Dialog.Title>
                <Dialog.Description>新しい企画区分を作成します</Dialog.Description>
              </Dialog.Header>
              <Dialog.Body>
                <Stack gap="4" w="full">
