...
describe("gen", () => {
  ...
  test("should work with mixed async operations", async () => {
    const asyncOp = async (x: number): Promise<Result.Result<number, string>> =>
      Result.succeed(x * 2);
  ...
  test("should handle type inference correctly", () => {
    ...
    expect(Result.isSuccess(result)).toBe(true);
    if (Result.isSuccess(result)) {
      expect(result.value).toEqual({ bool: true, num: 123, str: "hello" });
    }
  });
  ...
  describe("type tests", () => {
    ...
    test("should handle complex error types", () => {
      ...
      const divide = (a: number, b: number): Result.Result<number, DivisionError> => {
        if (b === 0) {
          return Result.fail({ type: "division_by_zero", divisor: b });
        }
        return Result.succeed(a / b);
      };
      ...
      const validate = (x: number): Result.Result<number, ValidationError> => {
        if (x < 0) {
          return Result.fail({ type: "validation_error", message: "negative" });
        }
    ...
    test("should preserve value types through composition", () => {
      const result = gen(function* ($) {
        ...
        const posts = yield* $(
          Result.succeed([{ title: "Post 1" }, { title: "Post 2" }] as { title: string }[]),
        );
        return { user, posts };
      });
