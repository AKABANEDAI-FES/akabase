...
CREATE TABLE `places` (
	...
	`created_at` integer DEFAULT (cast(unixepoch('subsecond') * 1000 as integer)) NOT NULL,
	FOREIGN KEY (`event_id`) REFERENCES `events`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`parent_id`) REFERENCES `places`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE INDEX `places_event_id_idx` ON `places` (`event_id`);--> statement-breakpoint
CREATE INDEX `places_parent_id_idx` ON `places` (`parent_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `places_event_parent_name_unique` ON `places` (`event_id`,`parent_id`,`name`);--> statement-breakpoint
...
CREATE UNIQUE INDEX `tags_event_name_unique` ON `tags` (`event_id`,`name`);--> statement-breakpoint
