...
export function useCanToggleBlockquote(editor: Editor | null, turnInto: boolean = true): boolean {
  const state = useEditorState({
    editor,
    selector(context) {
      const canToggle = canToggleBlockquote(context.editor, turnInto);
      return { canToggle };
    },
  });
...
/**
 * Checks if a blockquote is currently active
 */
export function isBlockquoteActive(editor: Editor | null): boolean {
  if (!editor || !editor.isEditable) return false;
  return editor.isActive("blockquote");
}
export function useIsBlockquoteActive(editor: Editor | null): boolean {
  const state = useEditorState({
    editor,
    selector(context) {
      const isActive = isBlockquoteActive(context.editor);
      return { isActive };
    },
  });
...
export function useBlockquote(config?: UseBlockquoteConfig) {
  ...
  const canToggle = useCanToggleBlockquote(editor);
  const isActive = useIsBlockquoteActive(editor);
